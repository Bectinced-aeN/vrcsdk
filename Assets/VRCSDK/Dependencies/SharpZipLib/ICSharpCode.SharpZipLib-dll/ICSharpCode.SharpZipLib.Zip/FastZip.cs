			...
			using (outputStream_ = new ZipOutputStream(outputStream))
			{
				...
				if (events_ != null)
				{
					if (events_.FileFailure != null)
					{
						...
						fileSystemScanner3.FileFailure = (FileFailureHandler)Delegate.Combine(fileSystemScanner3.FileFailure, events_.FileFailure);
					}
					if (events_.DirectoryFailure != null)
					{
						FileSystemScanner fileSystemScanner4 = fileSystemScanner;
						fileSystemScanner4.DirectoryFailure = (DirectoryFailureHandler)Delegate.Combine(fileSystemScanner4.DirectoryFailure, events_.DirectoryFailure);
					}
				}
