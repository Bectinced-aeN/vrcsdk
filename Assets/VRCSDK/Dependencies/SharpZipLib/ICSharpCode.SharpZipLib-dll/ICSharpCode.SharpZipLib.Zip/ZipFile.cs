	...
	public class ZipFile : IEnumerable, IDisposable
	{
		...
		private class UpdateComparer : IComparer<ZipUpdate>
		{
			public int Compare(ZipUpdate zx, ZipUpdate zy)
			{
				...
				{
					int num2 = (zx.Command != 0 && zx.Command != UpdateCommand.Modify) ? 1 : 0;
					int num3 = (zy.Command != 0 && zy.Command != UpdateCommand.Modify) ? 1 : 0;
					num = num2 - num3;
					if (num == 0)
					{
						long num4 = zx.Entry.Offset - zy.Entry.Offset;
		...
		private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
		{
			lock (baseStream_)
			{
				bool num = (tests & HeaderTest.Header) != (HeaderTest)0;
				bool num2 = (tests & HeaderTest.Extract) != (HeaderTest)0;
				...
				if (zipExtraData.Find(1))
				{
					...
					if ((num4 & 8) != 0)
					{
						if (num10 != -1 && num10 != entry.Size)
						{
							throw new ZipException("Size invalid for descriptor");
						}
						if (num9 != -1 && num9 != entry.CompressedSize)
						{
				...
				if ((num4 & 8) == 0 || ((num10 > 0 || num9 > 0) && entry.Size > 0))
				{
		...
		private Stream GetOutputStream(ZipEntry entry)
		{
			...
			case CompressionMethod.Deflated:
				return new DeflaterOutputStream(stream, new Deflater(9, noZlibHeaderOrFooter: true))
				{
					IsStreamOwner = false
				};
			default:
		...
		private void ReadEntries()
		{
			...
			{
				flag = true;
				if (LocateBlockWithSignature(117853008, num, 0, 4096) < 0)
				{
			...
			while (true)
			{
				...
				zipEntry.Crc = ((long)num13 & 4294967295L);
				zipEntry.Size = (num15 & uint.MaxValue);
				zipEntry.CompressedSize = (num14 & uint.MaxValue);
