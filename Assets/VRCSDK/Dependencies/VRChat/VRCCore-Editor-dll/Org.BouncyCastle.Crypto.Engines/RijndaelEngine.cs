	...
	internal class RijndaelEngine : IBlockCipher
	{
		...
		private long[][] GenerateWorkingKey(byte[] key)
		{
			...
			for (int j = 0; j < key.Length; j++)
			{
				...
				int num6 = j / 4;
				byte num8 = key[num4++];
				array3[num5, num6] = num8;
			}
			int num9 = 0;
			int num10 = 0;
			while (num10 < num3 && num9 < (ROUNDS + 1) * (BC / 8))
			{
				for (int k = 0; k < 4; k++)
				{
					array2[num9 / (BC / 8)][k] |= (long)(array[k, num10] & 0xFF) << num9 * 8 % BC;
				}
				num10++;
				num9++;
			}
			while (num9 < (ROUNDS + 1) * (BC / 8))
			{
				for (int l = 0; l < 4; l++)
				{
				...
				{
					for (int num12 = 1; num12 < 4; num12++)
					{
						for (int num13 = 0; num13 < 4; num13++)
						{
							array[num13, num12] ^= array[num13, num12 - 1];
						}
					}
					for (int num14 = 0; num14 < 4; num14++)
					{
						array[num14, 4] ^= S[array[num14, 3] & 0xFF];
					}
					for (int num15 = 5; num15 < num3; num15++)
					{
						for (int num16 = 0; num16 < 4; num16++)
						{
							array[num16, num15] ^= array[num16, num15 - 1];
						}
					}
				}
				int num17 = 0;
				while (num17 < num3 && num9 < (ROUNDS + 1) * (BC / 8))
				{
					for (int num18 = 0; num18 < 4; num18++)
					{
						array2[num9 / (BC / 8)][num18] |= (long)(array[num18, num17] & 0xFF) << num9 * 8 % BC;
					}
					num17++;
					num9++;
				}
