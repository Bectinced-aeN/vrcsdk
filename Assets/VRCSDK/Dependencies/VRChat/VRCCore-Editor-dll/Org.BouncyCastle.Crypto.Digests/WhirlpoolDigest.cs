...
namespace Org.BouncyCastle.Crypto.Digests
{
	internal sealed class WhirlpoolDigest : IDigest, IMemoable
	{
		...
		public WhirlpoolDigest()
		{
			...
			{
				int num = 8 * (i - 1);
				_rc[i] = ((C0[num] & -72057594037927936L) ^ (C1[num + 1] & 0xFF000000000000) ^ (C2[num + 2] & 0xFF0000000000) ^ (C3[num + 3] & 0xFF00000000) ^ (C4[num + 4] & 4278190080u) ^ (C5[num + 5] & 0xFF0000) ^ (C6[num + 6] & 0xFF00) ^ (C7[num + 7] & 0xFF));
			}
		}
		...
		{
			return (((long)(int)buffer[startPos] & 255L) << 56) | (((long)(int)buffer[startPos + 1] & 255L) << 48) | (((long)(int)buffer[startPos + 2] & 255L) << 40) | (((long)(int)buffer[startPos + 3] & 255L) << 32) | (((long)(int)buffer[startPos + 4] & 255L) << 24) | (((long)(int)buffer[startPos + 5] & 255L) << 16) | (((long)(int)buffer[startPos + 6] & 255L) << 8) | ((long)(int)buffer[startPos + 7] & 255L);
		}
