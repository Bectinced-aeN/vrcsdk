...
namespace Org.BouncyCastle.Crypto.Engines
{
	internal sealed class TwofishEngine : IBlockCipher
	{
		...
		{
			return gSBox[0 + 2 * (x & 0xFF)] ^ gSBox[1 + 2 * (((uint)x >> 8) & 0xFF)] ^ gSBox[512 + 2 * (((uint)x >> 16) & 0xFF)] ^ gSBox[513 + 2 * (((uint)x >> 24) & 0xFF)];
		}

		private int Fe32_3(int x)
		{
			return gSBox[0 + 2 * (((uint)x >> 24) & 0xFF)] ^ gSBox[1 + 2 * (x & 0xFF)] ^ gSBox[512 + 2 * (((uint)x >> 8) & 0xFF)] ^ gSBox[513 + 2 * (((uint)x >> 16) & 0xFF)];
		}
