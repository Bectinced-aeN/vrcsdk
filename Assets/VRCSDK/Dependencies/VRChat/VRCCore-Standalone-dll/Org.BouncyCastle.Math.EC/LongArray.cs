...
namespace Org.BouncyCastle.Math.EC
{
	internal class LongArray
	{
		...
		public BigInteger ToBigInteger()
		{
			...
			int num5 = 8 * (usedLength - 1) + num2;
			byte[] array2 = new byte[num5];
			for (int i = 0; i < num2; i++)
			{
				array2[i] = array[i];
			}
			for (int num6 = usedLength - 2; num6 >= 0; num6--)
			{
				long num7 = m_ints[num6];
				for (int num8 = 7; num8 >= 0; num8--)
				{
					array2[num2++] = (byte)((ulong)num7 >> 8 * num8);
				}
			}
		...
		private static long Interleave3_21to63(int x)
		{
			...
			return ((num3 & uint.MaxValue) << 42) | ((num2 & uint.MaxValue) << 21) | (num & uint.MaxValue);
		}
		...
		private static long Interleave3_13to65(int x)
		{
			...
			return ((num2 & uint.MaxValue) << 35) | (num & uint.MaxValue);
		}
		...
		private static long Interleave4_16to64(int x)
		{
			...
			return ((num2 & uint.MaxValue) << 32) | (num & uint.MaxValue);
		}
